...
const handleUpdateMemberInfo = () => {  
  ...
  const updatedInfo = {
    ...
    newAddress: member.address,    // 주소 (기존 주소를 그대로 사용)
 ...
 // 확인: updatedInfo가 제대로 준비됐는지 로그로 확인
 ...
 // 확인: updatedInfo가 제대로 준비됐는지 로그로 확인
 console.log('finalUpdatedInfo Info:', finalUpdatedInfo);
  ...
  <div class="update-member-info">
    ...
    <form @submit.prevent="handleUpdateMemberInfo">
      <div>
        ...
        <input type="text" id="name" v-model="member.name" disabled />
      </div>
